...
public class IconService {
 ...
 // 아이콘 목록 조회 (카테고리, 등급 필터 + 페이징)
 public IconPageVO getIconList(int page, String category, String rarity) {
     int size = 10;                          // 한 페이지당 개수
     int endRow = page * size;               
     int startRow = endRow - (size - 1);

     // 조건에 맞는 전체 아이콘 개수
     int totalCount = iconDao.countIcons(category, rarity);

     // 조건 + 페이징 적용 목록 조회
     List<IconDto> list =
         iconDao.selectListPaging(startRow, endRow, category, rarity);
 ...
 public IconDto drawRandomIcon(String memberId, long inventoryNo) { 
     ...
     if (gachaPool.isEmpty()) {
         throw new RuntimeException("뽑기 가능한 아이콘이 없습니다.");
     }
 ...
 public void equipIcon(String memberId, int iconId) {
     ...
     if (hasIcon == 0) {
         throw new RuntimeException("보유하지 않은 아이콘입니다.");
     }
